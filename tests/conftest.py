...
def _stub_tkinter():
    ...
    if module.__name__ == "tkinter" and isinstance(module, types.ModuleType) and module.__name__ not in sys.builtin_module_names:
        # Only remove if we created the stub
...
def _mock_ocr_fast(request):
    """
    Speed up fast tests by mocking OCR engines used in tests/scripts/test_ocr_accuracy.py.

    - If a test is marked with @pytest.mark.slow, do nothing (run real engines).
    - Otherwise, patch the getters in the script to return lightweight fakes.
    """
    if "slow" in request.keywords:
        # Respect slow tests: allow real OCR engines to run
        yield
        return

    # Build lightweight fake OCRs
    class _FakeWord:
    ...
    class _FakeYomiOCR:
        def __call__(self, *_args, **_kwargs):
            # Mimic yomitoku returning (OCRSchema, None)
            return (_FakeSchema(), None)

    class _FakePaddleOCR:
        def predict(self, *_args, **_kwargs):
            # Compatible with nested list format used in the test fallback path
            return [[(None, ("テスト", 0.99))]]

    # Patch the helper getters inside the accuracy test script
    with patch("tests.scripts.test_ocr_accuracy._get_yomitoku_ocr", return_value=_FakeYomiOCR()), \
         patch("tests.scripts.test_ocr_accuracy._get_paddle_ocr", return_value=_FakePaddleOCR()):
        yield
